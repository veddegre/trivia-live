...
import { PictureFinishImage } from "@/components/PictureFinishImage";
...
export function HostScreen({
  ...
  useEffect(() => {
    ...
    setWatchUrl(`${base}/watch/${encodeURIComponent(liveCode)}`);
  }, [liveCode]);
  ...
  useEffect(() => {
    ...
    const onConnect = () => {
      ...
      socket.emit(
        ...
        (res: { ok?: boolean; message?: string; state?: GamePublicState }) => {
          if (!res?.ok) {
            setError(res?.message || "Could not join as spectator");
            setState(null);
          } else {
            setError("");
            if (res.state) setState(res.state);
    ...
    const onReset = (payload: { code: string; hostToken?: string }) => {
      setError("");
      if (isHost && payload.hostToken && payload.code) {
        clearHostToken(code);
      ...
      if (payload.code) {
        ...
        router.replace(`/watch/${encodeURIComponent(payload.code)}`);
      }
    };
  ...
  function kickPlayer(row: LeaderboardEntry) {
    if (!confirm(
      `Remove “${row.name}” from the game?\n\nThey won’t be able to rejoin with that name until Play again.`
    ))
      return;
    const socket = getSocket();
    socket.emit(
          ...
          >
            ...
            {hasAudio && (
              <SpeedRevealAudio
                ...
                openedAt={state.questionOpenedAt}
                timeLimitSec={state.timeLimitSec}
                onPlayRequest={isHost ? startClock : undefined}
          ...
          <div className="mt-4 grid min-h-0 flex-1 gap-4 overflow-hidden lg:grid-cols-[1.35fr_1fr]">
            ...
            <aside className="panel min-h-0 overflow-y-auto rounded-2xl p-4 md:p-6">
              <StandingsList
                ...
                showDeltas
                title="In the lead"
                onRemove={isHost ? kickPlayer : undefined}
              />
          ...
          <header className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="text-right">
              <div className="text-sm font-bold uppercase tracking-[0.16em] text-amber">
                {state.endedRound
                  ? state.endedRound.title
                  : state.round
                    ? state.round.title
                    : `Question ${state.questionIndex} of ${state.questionTotal}`}
              </div>
          ...
          <div className="mt-8 grid flex-1 items-center gap-8 lg:grid-cols-[1.2fr_1fr]">
            <section className="flex flex-col items-center text-center">
              ...
              <p className="display mt-2 text-3xl md:text-4xl">
                {state.round
                  ? `${state.round.title} · question ${state.round.questionInRound + 1}`
              ...
              {isHost && (
                ...
                >
                  ▶{" "}
                  {state.round?.questionInRound === 0
                    ? `Start ${state.round.title}`
        ...
        <div className="mx-auto grid min-h-0 w-full max-w-[1500px] flex-1 grid-cols-1 px-6 py-6 md:px-10 md:py-8 lg:grid-cols-[minmax(0,1.7fr)_minmax(320px,1fr)]">
          ...
          >
            ...
            {state.gameType !== "TRIVIA" && (
              <p
                ...
                style={{ color: amber }}
              >
                {GAME_TYPE_LABEL[state.gameType]}
              </p>
            )}
            {state.rounds && state.rounds.length > 0 && (
              <p className="mt-3 text-center text-xs font-bold uppercase tracking-[0.18em] text-muted md:text-sm">
                {state.rounds.map((r) => r.title).join(" · ")}
              </p>
            )}

            {isHost && state.status === "DRAFT" && (
                  ...
                  <div className="pt-7">
                    ...
                    {isHost && watchUrl && (
                      <div className="pt-6">
                        ...
                        <p
                          className="mt-2.5 break-all font-mono text-[clamp(0.95rem,1.6vw,1.25rem)] font-semibold tracking-tight"
                          style={{ color: amber }}
                        >
                          {watchUrl}
                        </p>
          ...
          <aside className="flex min-h-0 flex-col pt-8 lg:pl-10 lg:pt-0">
            ...
            {isHost ? (
              ...
              <p className="mt-auto pt-8 text-xs font-bold uppercase tracking-[0.16em] text-muted">
                Watching — the host runs the game
              </p>
            )}
          </aside>
        </div>
